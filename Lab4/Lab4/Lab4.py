...
def translate_move_to_indexes(dungeon, current_player_pos, move_type):
    ...
    if(move_left == move_type):
        move_dictionary["row"] = row_pos;
        move_dictionary["col"] = col_pos - 1;
    elif(move_right == move_type):
        move_dictionary["row"] = row_pos;
        move_dictionary["col"] = col_pos + 1;
    elif(move_up == move_type):
    ...
    if(is_valid_move_type(move_type)):
        ...
        if(move_out_of_bounds(move_indexes) == False):
            ...
            if(check_if_collides(dungeon, move_indexes, trap_symbol)):
                ...
                handle_move(new_dungeon, prompt_move('Please enter a valid move character: '));
        else: 
...
def fill_dungeon(dungeon_structure, height):
    ...
    for col_num in range(len(dungeon_structure)):
        ...
        for height_num in range(height):
            dungeon_structure[col_num].append(empty_square_symbol);
...
def add_traps_to_blank_board(blank_dungeon, trap_number):
    ...
    while current_trap < trap_number:
        ...
        while trap_not_set == True:
            ...
            if(blank_dungeon[random_col][random_row] != trap_symbol):
                trap_not_set = False;
...
def add_player_and_treasure(trap_fill_dungeon):
    ...
    while treasure_created == False:
        ...
        if(trap_fill_dungeon[random_col][random_row] != trap_symbol and trap_fill_dungeon[random_col][random_row] != player_symbol):
            ...
            trap_fill_dungeon[random_col][random_row] = treasure_symbol;
    # return board
...
def main():
    ...
    print("the player, marked as " + player_symbol + ", " + "traverse the board towards the treasure, marked as " + treasure_symbol + ".");
    print("To win, you must safety move left, right, up, or down to the treasure, without touching the traps.")
    print("Note that the empty squares, marked as " + empty_square_symbol + " are safe to come in contact with, and should be the only squares you touch.");
    print();
    print("------------- Begin Game -------------");
    print();
